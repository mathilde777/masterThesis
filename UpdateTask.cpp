...
UpdateTask::UpdateTask(std::shared_ptr<Database> db)
    : db(db), matchedCluster(std::make_shared<std::vector<ClusterInfo>>()), errorClusters(std::make_shared<std::vector<ClusterInfo>>()) {}

void UpdateTask::execute(int trayId, Eigen::Vector3f ref) {
    this->trayId = trayId;
...
void UpdateTask::handleMatchedBoxes(std::shared_ptr<Database> db, const std::shared_ptr<Box>& box, std::vector<ClusterInfo>& matchedBoxes) {
    ...
    if (matchedBoxes.empty()) {
        ...
        for (auto it = matchedBoxes.begin(); it != matchedBoxes.end();) {
            ...
            auto PNGPathCropped = photoProcessing->findLatestCroppedImage();
            std::cout << "Cropped Path: " << (PNGPathCropped ? PNGPathCropped->c_str() : "Error: No PNG file found") << std::endl;
            if (!PNGPathCropped) {
                std::cout << "Error: No cropped PNG file found" << std::endl;
                break;
            }
...
void UpdateTask::handleErrorBoxes(std::shared_ptr<Database> db) {
    ...
    if (errorClusters->size() == 1 && errorBoxes.size() == 1) {
        std::cout << "UPDATE!" << errorBoxes.size() << std::endl;
        db->updateBox((*errorBoxes.begin())->getId(), errorClusters->begin()->centroid.x(), errorClusters->begin()->centroid.y(), errorClusters->begin()->centroid.z(),
                      errorClusters->begin()->dimensions.x(), errorClusters->begin()->dimensions.y(), errorClusters->begin()->dimensions.z());
        matchedCluster->push_back(*it);
